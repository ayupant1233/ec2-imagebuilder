...
data "aws_iam_policy_document" "allow_access_from_pipeline_service_role" {
  ...
  statement {
    principals {
      type        = "AWS"
      identifiers = ["arn:aws:iam::${var.account_id}:role/${var.ecr_test_role_name}"]
    }
...
resource "aws_imagebuilder_image_pipeline" "ecr-test" {
  ...
  tags = {
    "Name" = "${var.image_name}-ecr_test-container"
  }

  depends_on = [
    aws_imagebuilder_container_recipe.container_image,
    aws_imagebuilder_infrastructure_configuration.ecr-test,
