...
resource "aws_cloudwatch_event_rule" "inspector_finding" {
  name        = "ecr_test-image-build"
  description = "Trigger ecr_test Image Re-Build on High or Critical Severity Findings"
...
resource "aws_cloudwatch_event_target" "container_pipeline" {
  ...
  arn       = aws_imagebuilder_image_pipeline.ecr-test.arn
